...
async def user_activate(request):
    ...
    return response.json({"success": "user activate please login"})


async def user_login(request):
    try:
        user = abstract.LoginUser(**request.json)
        login_res = await messagebus.handle(
        ...
        result = login_res.pop(0)
        request.ctx.session[result["user"]["id"]] = result["user"]
        return response.json({"token": result["token"]})
    except exceptions.INVALIDEMAILPASSWORD:
...
class UserRoutes(HTTPMethodView):
    async def get(self, request, user_id):
        ...
        if user_id:
            # q_model.id_ = user_id
            id_ = user_id
            user_res = await views.get_user_by_uuid(id_, request.app.ctx.db)
            # return response.json(jsonable_encoder(user))
            res = [jsonable_encoder(user_res)]
        else:
            user_res = await views.get_users(request.app.ctx.db, page=page)
            res = jsonable_encoder(user_res)
        user = []
        for item in res:
            item.pop("created_at")
