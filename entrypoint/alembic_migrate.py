...
class AlembicCommand:
    def __init__(self):
        self.alembic_paths = glob.glob(
            f"{str(Path(product_components.__file__).parent)}/**/alembic.ini"
        )
        self.alembic_paths.extend(
            glob.glob(f"{str(Path(user_components.__file__).parent)}/**/alembic.ini")
        )
        self.alembic_paths.extend(
            glob.glob(f"{str(Path(order_components.__file__).parent)}/**/alembic.ini")
        )
